...
const GROQ_MODEL_2 = "qwen/qwen3.6-27b";

// TIMEOUT FOR DIFFERENT MODELS
...
QUESTION
==================================================
The question field must preserve the complete visible question.
...
Simple: 1-4 essential steps.
Medium: 3-7 essential steps.
Hard: Only the essential reasoning required to understand the answer.
...
DIFFICULTY
==================================================
Easy: Direct formula, definition, one-step calculation.
Medium: Multiple steps or moderate reasoning.
Hard: Complex reasoning or multiple concepts.
...
function validateImage(image: unknown): {
  ...
  if (
    value.length > MAX_IMAGE_STRING_LENGTH
  ) {
    return {
      valid: false,
  ...
  const match = value.match( /^data:(image\/(?:png|jpeg|jpg|webp|gif));base64,([A-Za-z0-9+/=\s]+)$/ );
  if (!match) {
    return {
      valid: false,
  ...
  const estimatedBytes = Math.floor( (base64Data.length * 3) / 4 ) - (base64Data.endsWith("==") ? 2 : base64Data.endsWith("=") ? 1 : 0);
...
function validateAndNormalize(
  ...
  const difficulty = getString(raw.difficulty);
  const answer = getString(raw.answer);
  const explanation = getString(raw.explanation);
  const similarQuestion = getString(raw.similar_question);
  ...
  const finalDifficulty = normalizedDifficulty
      .charAt(0)
      .toUpperCase() +
    ...
    messages: [
      ...
      },
      {
        ...
        content: [
          {
            type: "text",
            text: "Analyze the COMPLETE uploaded educational question image. Solve EVERY visible question and sub-question. Return ONLY the required JSON object. Make all mathematical expressions valid Markdown LaTeX.",
          },
          {
            type: "image_url",
            image_url: {
...
//Saving Analysis to database history
async function saveAnalysisToHistory(
  ...
  try {
    ...
    console.log("[Snap2Study] Analysis saved to history.");
  } catch (error) {
    console.error( "[Snap2Study] Failed to save analysis to history:", error
    );
  }
}
...
async function requestProvider(
  ...
  > {
  console.log(`[Snap2Study] ${provider}: trying ${model}`);
  ...
  try {
    const response = await fetchWithTimeout(
        url,
        {
          ...
          headers: { Authorization: `Bearer ${apiKey}`,
            ...
            "HTTP-Referer": "https://snap2study.vercel.app",
            "X-Title": "Snap2Study",
          },
          body: JSON.stringify(
            buildRequestBody(
    ...
    console.log(`[Snap2Study] ${provider} HTTP: ${response.status}`);
    ...
    try { data = await response.json();
      ...
      return {
        success: false,
        reason: "Invalid HTTP JSON response",
      };
    }

    console.log(`[Snap2Study] ${provider} model: ${ data?.model ?? model }` );
    console.log(`[Snap2Study] ${provider} provider: ${ data?.provider ?? "unknown" }` );

    if (isProviderError(data)) {
      const errorMessage = getErrorMessage(data);
      console.error(`[Snap2Study] ${provider} provider error: ${errorMessage}` );
    ...
    if (!response.ok) {
      const errorMessage = getErrorMessage(data);
      console.error(`[Snap2Study] ${provider} HTTP error: ${errorMessage}`);
    ...
    console.log(`[Snap2Study] ${provider} finish reason: ${ finishReason ?? "undefined" }` );
    console.log(`[Snap2Study] ${provider} native finish reason: ${ nativeFinishReason ?? "undefined" }` );
    ...
    if ( typeof content !== "string" || !content.trim()
      ...
      return {
        success: false,
        reason: "AI returned no usable content",
      };
    }
    console.log(`[Snap2Study] ${provider} AI content received.`);

    const cleaned = cleanAIContent(content);
    console.log(`[Snap2Study] ${provider} cleaned JSON length: ${cleaned.length}`);

    let parsed: any;
    try { parsed = JSON.parse(cleaned);
    } catch (error) {
      console.error(`[Snap2Study] Invalid JSON from ${provider} ${model}.`);
      console.error("[Snap2Study] JSON parse error:", error );
      console.error("[Snap2Study] Content:", cleaned.slice(0, 2500));
      return {
        success: false,
    ...
    if (!result) {
      console.error(`[Snap2Study] ${provider} local validation failed.`);
      console.error("[Snap2Study] Received keys:", Object.keys( parsed ?? {} ));
      return {
        success: false,
        reason: "Invalid or incomplete result structure",
      };
    }

//Detecting number of questions
    const detectedQuestionCount = getQuestionCount( result.question );
    const answerQuestionCount = getQuestionCount( result.answer );
    const explanationQuestionCount = getQuestionCount( result.explanation );

    console.log(`[Snap2Study] ${provider} question count: ${detectedQuestionCount}`);
    console.log(`[Snap2Study] ${provider} answer count: ${answerQuestionCount}`);
    console.log(`[Snap2Study] ${provider} explanation count: ${explanationQuestionCount}`);
    ...
    if (
      ...
      console.error(`[Snap2Study] ${provider}: answer appears incomplete.`);
      return {
        success: false,
    ...
    if (
      ...
      console.error(`[Snap2Study] ${provider}: explanation appears incomplete.`);
      return {
        success: false,
        reason: "Explanation appears incomplete",
      };
    }

    console.log(`[Snap2Study] SUCCESS: ${provider} → ${model}`);
    return {
      success: true,
      result,
      model,
    ...
    if (
      ...
      console.error(`[Snap2Study] ${provider} timeout after ${timeoutMs}ms.`);
      return {
        success: false,
        reason: `Request timeout after ${timeoutMs}ms`,
      };
    }
    console.error(`[Snap2Study] ${provider} request failed: ${errorMessage}`);
    return {
      success: false,
      reason: errorMessage,
    };
...
export async function POST(
  ...
  console.log("[Snap2Study] Starting analysis...");
  try {
    if (
      request.method !== "POST"
    ) {
    ...
    console.log("[Snap2Study] Rate limit:",
      {
  ...
  // User Authentication
    ...
    if (!currentUser) {
      console.log("[Snap2Study] Analysis rejected: user not authenticated.");

      return NextResponse.json(
        {
          error: "You must be logged in to analyze a question.",
          code: "AUTH_REQUIRED",
        },
        {
          status: 401,
    ...
    console.log(`[Snap2Study] Authenticated user: ${currentUser.email}`);
   
    const authenticatedUser = currentUser;

    console.log(`[Snap2Study] Authenticated user: ${authenticatedUser.email}`);
  ...
  //API KEYS IMPLEMENTATION
    ...
    const openRouterKey = process.env.OPENROUTER_API_KEY_2;

    console.log("[Snap2Study] Providers available:",
      {
        groqAccount1: Boolean(groqKey1),
        groqAccount2: Boolean(groqKey2),
        groqAccount3: Boolean(groqKey3),
        groqAccount4: Boolean(groqKey4),
        openRouterAccount2: Boolean(openRouterKey),
      }
    );

  // Checking AI provider KEY availability
    if (
      !groqKey1 &&
      !groqKey2 &&
      !groqKey3 &&
      ...
      console.error("[Snap2Study] No AI provider keys configured.");
      return NextResponse.json(
        {
  ...
  // Analysis histury save
    ...
    async function handleSuccess(
      ...
      return NextResponse.json(
        result.result,
        {
          status: 200,
          headers: {
            "X-Snap2Study-Provider": result.provider,
            "X-Snap2Study-Model": result.model,
    ...
    if (groqKey1) {
      ...
      failures.push(`Groq Account 1: ${result.reason}`);
    } else {
      failures.push("Groq Account 1: API key missing");
    }
    ...
    if (groqKey2) {
      ...
      failures.push(`Groq Account 2: ${result.reason}`);
    } else {
      failures.push("Groq Account 2: API key missing");
    }
    await wait( FALLBACK_DELAY_MS );
    ...
    if (groqKey3) {
      ...
      failures.push(`Groq Account 3: ${result.reason}`);
    } else {
      failures.push("Groq Account 3: API key missing");
    }
    await wait( FALLBACK_DELAY_MS );
    ...
    if (groqKey4) {
      ...
      failures.push(`Groq Account 4: ${result.reason}`);
    } else {
      failures.push("Groq Account 4: API key missing");
    }
    await wait( FALLBACK_DELAY_MS );
    ...
    if (openRouterKey) {
      ...
      failures.push(`OpenRouter Nemotron: ${result.reason}`);
    } else {
      failures.push("OpenRouter Nemotron: API key missing");
    }

//Analysis Failed by all models
    console.error("[Snap2Study] All AI providers failed.");

    console.error("[Snap2Study] Failure summary:", failures );
    ...
    console.error("[Snap2Study] Global API error:", error);
    return NextResponse.json(
      {
